...
namespace util = ingenuity::NodeEditor::Utils;

using namespace ax;

using ingenuity::Widgets::IconType;
...
struct Pin
{
	...
	Pin(int id, const char* name, PinType type) :
		ID(id), Node(nullptr), Name(name), Type(type), Kind(PinKindInGen::Input)
	{
	}
};
...
struct LinkInGen
{
	...
	LinkInGen(ed::LinkId id, ed::PinId startPinId, ed::PinId endPinId) :
...
static std::vector<LinkInGen>    s_Links;
...
static LinkInGen * FindLink(ed::LinkId id)
{
...
static void BuildNode(Node * node)
{
	...
	{
		input.Node = node;
		input.Kind = PinKindInGen::Input;
	}

	for (auto& output : node->Outputs)
	{
		output.Node = node;
		output.Kind = PinKindInGen::Output;
	}
}
...
void Application_Initialize()
{
	...
	s_Links.push_back(LinkInGen(GetNextLinkId(), s_Nodes[5].Outputs[0].ID, s_Nodes[6].Inputs[0].ID));
	s_Links.push_back(LinkInGen(GetNextLinkId(), s_Nodes[5].Outputs[0].ID, s_Nodes[7].Inputs[0].ID));

	s_Links.push_back(LinkInGen(GetNextLinkId(), s_Nodes[14].Outputs[0].ID, s_Nodes[15].Inputs[0].ID));
...
void DrawPinIcon(const Pin & pin, bool connected, int alpha)
{
	...
	ingenuity::Widgets::Icon(ImVec2(s_PinIconSize, s_PinIconSize), iconType, connected, color, ImColor(32, 32, 32, alpha));
};
		...
		if (!createNewNode)
		{
			if (ed::BeginCreate(ImColor(255, 255, 255), 2.0f))
			{
				...
				if (ed::QueryNewLink(&startPinId, &endPinId))
				{
					...
					if (startPin->Kind == PinKindInGen::Input)
					{
					...
					if (startPin && endPin)
					{
						...
						else
						{
							...
							{
								s_Links.emplace_back(LinkInGen(GetNextId(), startPinId, endPinId));
...
# if 1
	...
	if (ImGui::BeginPopup("Create New Node"))
	{
		...
		if (node)
		{
			...
			{
				auto& pins = startPin->Kind == PinKindInGen::Input ? node->Outputs : node->Inputs;
				...
				for (auto& pin : pins)
				{
					if (CanCreateLink(startPin, &pin))
					{
						...
						s_Links.emplace_back(LinkInGen(GetNextId(), startPin->ID, endPin->ID));
						s_Links.back().Color = GetIconColor(startPin->Type);
